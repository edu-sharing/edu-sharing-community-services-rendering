    ...
    extends ESRender_Plugin_Abstract
{
	...
	if($contentNode->getProperty('{http://www.campuscontent.de/model/1.0}replicationsource') === 'serlo') {
	    ...
	    $dialog = $dataProtectionRegulationHandler->getApplyDataProtectionRegulationsDialog($contentNode->id, 'Serlo', 'https://de.serlo.org/datenschutz', 'VIDEO_DEFAULT');
	    ...
	    Config::set('urlEmbedding', $dialog.'<iframe id="videoWrapperInner_'.$contentNode->id.'" style="display:none" src="https://de.serlo.org/'.$replicationSourceId.'?contentOnly&hideBreadcrumbs" width="100%" height="800px" border="0"></iframe>');
	}
    }
}
