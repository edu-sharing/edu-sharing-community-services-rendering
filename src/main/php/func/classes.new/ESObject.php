...
class ESObject {
    ...
    public function getPositionInConversionQueue($format, $resolution = null) {
        ...
        try {
            ...
            $sql = 'SELECT COUNT("ESOBJECT_CONVERSION_ID") AS "POS" FROM "ESOBJECT_CONVERSION" WHERE
            "ESOBJECT_CONVERSION_STATUS" = :status AND ' .
            (($resolution) ? '"ESOBJECT_CONVERSION_RESOLUTION" = :resolution AND ' : '') . '
            "ESOBJECT_CONVERSION_ID" <
                ( SELECT "ESOBJECT_CONVERSION_ID" FROM "ESOBJECT_CONVERSION" WHERE
                  "ESOBJECT_CONVERSION_OBJECT_ID" = :objectid AND "ESOBJECT_CONVERSION_FORMAT" = :format' . (($resolution) ? ' AND "ESOBJECT_CONVERSION_RESOLUTION" = :resolution' : '') . '
                )';
            $stmt = $pdo -> prepare($sql);
            $stmt -> bindValue(':objectid', $this->id, PDO::PARAM_INT);
            $stmt -> bindValue(':format', $format);
